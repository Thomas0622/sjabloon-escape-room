...
include_once "config.php";
 ...
 function crudMain(){
    ...
    <h1>Crud Riddles</h1>
    <nav>
		<a href='insert.php'>Toevoegen nieuwe riddle</a>
    </nav><br>";
    echo $txt;

    // Haal alle riddles record uit de tabel 
 ...
 // selecteer de rij van de opgeven id uit de table riddles
...
function updateRecord($row){
    ...
    $stmt->execute([
        ...
        ':hint'=>$row['hint'],
        ':roomid'=>$row['roomid'],
        ':timelimit(seconds)'=>$row['timelimit(seconds)'],
        ':team'=>$row['team'],
        ':id'=>$row['id']
    ]);
...
function insertRecord($post){
    ...
    // Maak een query 
    $sql = "
        INSERT INTO " . CRUD_TABLE . " (riddle, answer, hint, roomid, timelimit(seconds), team)
        VALUES (:riddle, :answer, :hint, :roomid, :timelimit(seconds), :team) 
    ";
    ...
    $stmt->execute([
        ...
        ':hint'=>$_POST['hint'],
        ':roomid'=>$_POST['roomid'],
        ':timelimit(seconds)'=>$_POST['timelimit(seconds)'],
        ':team'=>$_POST['team']
    ]);
